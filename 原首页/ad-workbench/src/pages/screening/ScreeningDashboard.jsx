...
import {
  ...
  ArrowDownRight, Minus, Info, X, ChevronUp, ChevronLeft
} from 'lucide-react';
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary" onClick={onCreateProject}><Plus size={16} style={{ marginRight: 4 }} />新建项目</button>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          <StatCard title="总预算" value={`¥${(project.budget / 10000).toFixed(0)}万`} icon={DollarSign} color="blue" />
          <StatCard title="目标达人" value={project.creatorCount} subtitle="人" icon={Users} color="purple" />
          <StatCard title="已通过" value={stats.passed} subtitle={`/${stats.total}人`} icon={UserCheck} color="green" />
...
function ScreeningReviewTab({ project, screeningStatus, setScreeningStatus }) {
  ...
  const handleReview = (creator, action) => {
...
function ProjectSetupTab({ project }) {
    ...
    <div>
      ...
      {activeSection === 'info' && (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <button className="btn btn-primary" onClick={() => setSaved(true)}><Save size={14} style={{ marginRight: 4 }} />保存立项信息</button>
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <Badge variant={project.screeningPlan?.briefType === 'complex' ? 'amber' : 'blue'}>
                {project.screeningPlan?.briefType === 'complex' ? '复杂需求' : '简单需求'}
              </Badge>
            </div>
            {project.screeningPlan?.briefType ? (
              <div>
                {/* 硬性条件 */}
                <div style={{ marginBottom: 20 }}>
                  ...
                  {(project.screeningPlan.hardFilters || []).length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {(project.screeningPlan.hardFilters || []).map((f, i) => (
                ...
                <div>
                  ...
                  {Object.keys(project.screeningPlan.scoringWeights || {}).length > 0 ? (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                      {Object.entries(project.screeningPlan.scoringWeights || {}).map(([key, weight]) => {
              ...
              <div style={{ textAlign: 'center', padding: 32 }}>
                ...
                <button className="btn btn-primary"><Sparkles size={14} style={{ marginRight: 4 }} />AI 生成筛选标准</button>
              </div>
            )}
          </div>
          ...
          {/* Brief 输入区 */}
          <div className="card">
            <h4 style={{ margin: '0 0 16px', color: '#E8ECF1' }}>客户 Brief</h4>
            <div style={{ padding: 16, background: '#12161C', borderRadius: 8, borderLeft: '3px solid #8B5CF6', marginBottom: 16 }}>
              <p style={{ margin: 0, color: '#C8CDD5', fontSize: 14, lineHeight: 1.7 }}>{project.brief?.description || '暂无 Brief 描述'}</p>
            </div>
            <button className="btn btn-secondary"><Sparkles size={14} style={{ marginRight: 4 }} />重新生成筛选标准</button>
          </div>
        </div>
      )}
      ...
      {activeSection === 'feishu' && (
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h4 style={{ margin: 0, color: '#E8ECF1' }}>飞书多维表格绑定</h4>
            ...
            {project.feishuBinding?.linked ? (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 20 }}>
                  <div className="info-row"><span className="info-label">表格链接</span><span className="info-value" style={{ fontSize: 12, wordBreak: 'break-all' }}>{project.feishuBinding.tableUrl}</span></div>
                  <div className="info-row"><span className="info-label">目标表名</span><span className="info-value">{project.feishuBinding.tableName}</span></div>
                  <div className="info-row"><span className="info-label">Base Token</span><span className="info-value" style={{ fontFamily: 'monospace' }}>{project.feishuBinding.baseToken}</span></div>
                  <div className="info-row"><span className="info-label">Table ID</span><span className="info-value" style={{ fontFamily: 'monospace' }}>{project.feishuBinding.tableId}</span></div>
                </div>
                      ...
                      <tbody>
                        {(project.feishuBinding.fieldMapping || []).map((fm, i) => (
                          <tr key={i} style={{ borderBottom: '1px solid #1A1F2B' }}>
                            ...
                            <td style={{ padding: '8px 12px', color: '#C8CDD5' }}>{fm.feishu}</td>
                            <td style={{ padding: '8px 12px' }}><Badge variant="default">{fm.type}</Badge></td>
                            <td style={{ padding: '8px 12px' }}>
                              <Badge variant={fm.writable ? 'green' : 'red'}>{fm.writable ? '可写' : '只读'}</Badge>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <button className="btn btn-primary"><Send size={14} style={{ marginRight: 4 }} />写回飞书</button>
                  <button className="btn btn-secondary"><RefreshCw size={14} style={{ marginRight: 4 }} />重新读取字段</button>
                </div>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: 32 }}>
                <Link2 size={32} style={{ color: '#5A6478', marginBottom: 12 }} />
                <p style={{ color: '#8B95A5', marginBottom: 16 }}>尚未绑定飞书多维表格</p>
                <div style={{ maxWidth: 400, margin: '0 auto', display: 'grid', gap: 12 }}>
                  <input className="input-field" placeholder="粘贴飞书多维表格链接..." />
                  <button className="btn btn-primary"><Link2 size={14} style={{ marginRight: 4 }} />绑定飞书表格</button>
                </div>
              </div>
            )}
          </div>
...
function ScreeningDashboard() {
  ...
  const renderTabContent = () => {
    ...
    if (!currentProject) {
      return (
        <div className="card" style={{ textAlign: 'center', padding: 48 }}>
          ...
          <button className="btn btn-primary" onClick={() => setActiveTab('projects')}>前往项目预览</button>
        </div>
      );
    }
    ...
    switch (activeTab) {
      ...
      case 'screening-review': return <ScreeningReviewTab project={currentProject} screeningStatus={screeningStatus} setScreeningStatus={setScreeningStatus} />;
      case 'score-preview': return <ScorePreviewTab project={currentProject} />;
      case 'project-setup': return <ProjectSetupTab project={currentProject} />;
